...
def optimize_weights(
    ...
    *,
    ...
    if OPT.include_txn_costs_in_objective:
        ...
        ratio = cp.abs(dollar_change) / adv_safe
        if psi >= 1.0:
            impact_terms = cp.power(ratio, psi)
        else:
            # cp.power(x, p) is concave for 0 < p < 1 which breaks DCP; fall back to
            # a linear penalty in that regime so the optimization remains convex.
            impact_terms = ratio
        impact_piece = cp.sum(cp.multiply(kappa, impact_terms))
